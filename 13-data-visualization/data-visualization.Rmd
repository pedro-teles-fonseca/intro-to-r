...
- The `group` aesthetic does not add a legend or any other distinguishing features to the geoms.
...
- For less than 1000 obsetvations the method is `"loess"` (a local polynomial regression).

- Admissable methods are: `"auto"`, `"lm"`, `"glm"`, `"gam"` and `"loess"`. 
...
- You can use the same logic to specify a different dataset for each layer. 
...
- While it makes your graph less accurate at small scales, it makes it more revealing at large scales.
...
gplot(data = mpg, aes(x = cty, y = hwy)) +
        geom_jitter()
...
The `labs()` function also allows us to to add a subtitio and/or a caption:
...
You can save graphs in other locations by providing the path to `ggsave()`: 
...
You can use the `plot` argument of `ggsave()` to save any graph that is assigned to an objects: 
