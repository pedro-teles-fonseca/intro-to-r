...
We add labels to a graph with the `labs()` function:
...
- The background of teh graphs can be costumized with a theme:
...
![The eight themes built-in to ggplot2.](themes.png)

## Saving plots

`ggsave()` will save the most recent plot to the working directory. The name of the output file is provided as input to `ggsave()`:
...
You can use the `plot` argument to save graphs that are assigned to an objects: 
