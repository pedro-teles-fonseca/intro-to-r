...
We add labels to a graph with the \texttt{labs()} function:
...
\begin{center}\includegraphics[height=150px]{data-visualization_files/figure-beamer/unnamed-chunk-151-1} \end{center}
...
\begin{figure}
\centering
\includegraphics{themes.png}
\caption{The eight themes built-in to ggplot2.}
\end{figure}
...
directory. The name of the output file is provided as input to
\texttt{ggsave()}:
...
You can use the \texttt{plot} argument to save graphs that are assigned
to an objects:
...
The default value of the \texttt{plot} argument is \texttt{last\_plot()}
